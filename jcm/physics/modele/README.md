...
2. Port the convective **trigger / cloud base** (`CLOUD_BASE` /
   `cloud_base_closure`) on top of those primitives, then wire the result into
   `GissConvection.__call__` so it stops returning zero.
3. Replace the trivial all-zero DYCOMS fixture with **nonzero BOMEX/RICO targets**
