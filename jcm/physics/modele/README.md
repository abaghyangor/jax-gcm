...
1. Port the simpler, self-contained pieces first — saturation/moist-static-energy
   thermodynamics, then the convective **trigger / cloud base**
   (`CLOUD_BASE` / `cloud_base_closure`) — with standalone unit + gradient tests
   in the style of the other JCM convection schemes.
2. Replace the trivial all-zero DYCOMS fixture with **nonzero BOMEX/RICO targets**
