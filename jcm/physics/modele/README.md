...
5. Wire trigger + closure into `GissConvection.__call__` (needs the column's
   pressure/Exner profile from the coordinate system) for a first end-to-end
   cloud-base diagnostic, then continue up the plume (lift/entrainment,
   condensation, downdrafts, precipitation).
6. Replace the trivial all-zero DYCOMS fixture with **nonzero BOMEX/RICO targets**
